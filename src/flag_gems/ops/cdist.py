...
def _cdist_backward_kernel(
    ...
    # For each feature dim we accumulate grad_x1 over all n2 elements.
    # MODE mirrors the ATen `_cdist_backward` dispatch and selects the
    # gradient formula of the p-norm distance:
    #   MODE=1 (p == 1.0): grad * sign(diff)
    #   MODE=2 (p == 2.0): grad * diff / dist               (dist == 0 -> 0)
    #   MODE=3 (p == inf): grad * sign(diff) * (|diff| == dist)
    #   MODE=0 (general) : sign(diff) * |diff|^(p-1) * grad / dist^(p-1)
    #                      (dist == 0 or diff == 0 -> 0)
    # where diff = x1[b, n1_idx, d] - x2[b, n2_j, d] and dist = cdist[b, n1_idx, n2_j].
    ...
    for n2_start in range(0, n2, BLOCK_N2):
        ...
        # sign(diff): 1.0 if diff > 0, -1.0 if diff < 0, 0.0 otherwise
        sign_diff = tl.where(diff > 0.0, 1.0, tl.where(diff < 0.0, -1.0, 0.0))

        # dist is [BLOCK_N2, 1]; broadcast across the feature dim
        dist = cdist[:, None]

        # grad is [BLOCK_N2]; broadcast across dim dimension
        if MODE == 1:  # p == 1.0 : grad * sign(diff)
            contrib = grad[:, None] * sign_diff
        elif MODE == 2:  # p == 2.0 : grad * diff / dist (dist == 0 -> 0)
            contrib = tl.where(dist == 0.0, 0.0, grad[:, None] * diff / dist)
        elif MODE == 3:  # p == inf : grad * sign(diff) * (|diff| == dist)
            is_max = tl.where(abs_diff == dist, 1.0, 0.0)
            contrib = grad[:, None] * sign_diff * is_max
        else:  # general p: sign(diff) * |diff|^(p-1) * grad / dist^(p-1)
            # ATen returns 0 for dist == 0 and for diff == 0 (the latter also
            # avoids pow(0, negative) when p < 1). Guarded lanes are replaced
            # with 1.0 so that the pow below stays finite and is masked out.
            guard = (abs_diff == 0.0) | (dist == 0.0)
            abs_diff_safe = tl.where(guard, 1.0, abs_diff)
            dist_safe = tl.where(guard, 1.0, dist)
            p_m1 = p - 1.0
            numerator = (
                sign_diff * libdevice.pow(abs_diff_safe, p_m1) * grad[:, None]
            )
            denominator = libdevice.pow(dist_safe, p_m1)
            contrib = tl.where(guard, 0.0, numerator / denominator)
...
def _cdist_backward(grad, x1, x2, p, cdist):
    ...
    # Mirror ATen: the zero-norm (p == 0) gradient is identically zero.
    if p == 0.0:
        return torch.zeros_like(x1)

    # Select the p-specific backward formula (mirrors the ATen CUDA dispatch).
    if p == 1.0:
        mode = 1
    elif p == 2.0:
        mode = 2
